    ...
    *)                                                       
    let rec eval env ((st, i, o, r) as conf) expr = match expr with
      ...
      | Binop (op, x, y) -> let (_, _, _, Some firstArg) as conf = eval env conf x in
        ...
        in (st, i, o, Some (calculateExpression op firstArg secondArg)) 
...
module Stmt =
  struct
    ...
    *)
    ...
    let rec parseElseIfActions elseIfActions elseAction =  match elseIfActions with
      | [] -> elseAction
      | (condition, action)::tailElseIfActions -> If (condition, action, parseElseIfActions tailElseIfActions elseAction)

    let parseElse elseIfActions elseAction = 
      let elseActionParsed = match elseAction with
        | None -> Skip
        | Some action -> action
    in parseElseIfActions elseIfActions elseActionParsed
    ...
    ostap (
      ...
      | %"if" condition: !(Expr.parse) %"then" action:parse 
        elseIfActions:(%"elif" !(Expr.parse) %"then" parse)*
        elseAction:(%"else" parse)?
        %"fi"                                              { If (condition, action, parseElse elseIfActions elseAction)}
